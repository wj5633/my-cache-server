...
func run(client cache.Client, c *cache.Cmd, r *result) {
	expect := c.Value
	start := time.Now()
	client.Run(c)
...
func pipeline(client cache.Client, cmds []*cache.Cmd, r *result) {
...
func operate(id, count int, ch chan *result) {
	client := cache.New(typ, server)
	cmds := make([]*cache.Cmd, 0)
	...
	for i := 0; i < count; i++ {
		...
		c := &cache.Cmd{name, key, value, nil}
		if pipelen > 1 {
			cmds = append(cmds, c)
			if len(cmds) == pipelen {
